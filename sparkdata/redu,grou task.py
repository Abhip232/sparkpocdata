...
res=aslrdd.filter(lambda x: "age" not in x).map(lambda x:x.split(",")).map(lambda x:(x[2],1))\
...
for i in res.collect():
    print(i)
'''Map: apply a logic on top of each & every element ... 
how many input elements u have same number of output elements u ll get.
